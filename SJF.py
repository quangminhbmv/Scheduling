...
class SJF:
    ...
    def run(self):
        ...
        while cpu_queue or resource_waiting_queue:
            ...
            # Chọn tiến trình đến và còn CPU burst để chạy
            ready_processes = [p for p in cpu_queue if p.arrival_time <= t and p.current_cpu_index < len(p.cpu_times)]

            if ready_processes:
                # Chọn tiến trình có CPU burst nhỏ nhất
                actual_p = min(ready_processes, key=lambda p: p.cpu_times[p.current_cpu_index])
...
def read_input_file(filename):
    ...
    with open(filename, 'r') as file:
        ...
        algorithm_choice = int(lines[0].strip())  # Dòng 1: chọn thuật toán
        n = int(lines[1].strip())  # Dòng 2: số tiến trình

        for i in range(2, 2 + n):
            parts = lines[i].strip().split()
            process_name = f"{i - 1}"  # Tạo tên tiến trình tự động: P1, P2, P3...
            arrival_time = int(parts[0])
            burst_info = []
            for item in parts[1:]:
                if "(" in item and ")" in item:
                    burst_info.append(item)
                else:
                    burst_info.append(int(item))
            p_list.append((process_name, arrival_time, *burst_info))
    return algorithm_choice, p_list

algorithm_choice, p_list = read_input_file("input.txt")

resources = {"R": 16, "R1": 16, "R2": 16}

if algorithm_choice == 3:
    scheduler = SJF(p_list, resources)
    scheduler.run()
else:
    print("Thuật toán chưa được hỗ trợ!")
